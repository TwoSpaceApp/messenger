...
3. **Управление состоянием**: Повсеместно используется фреймворк Riverpod (`ConsumerWidget`, `ConsumerStatefulWidget`). Старайтесь не создавать глобальные переменные состояния вне провайдеров.