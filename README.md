...
Мы планируем объединить лучшие функции **Telegram**, **Discord** и **экранных виджетов**, создавая интуитивный, гибкий и красивый инструмент для общения с новыми решениями в условиях развития современного интернета.
...
TwoSpace использует **Aegis** — собственный бинарный протокол поверх TCP, поверх которого клиент поднимает зашифрованную сессию и синхронизирует сообщения, комнаты, участников и служебные события.

Практически это означает следующее:

- клиент устанавливает TCP-соединение с Aegis-сервером и может работать как в обычном режиме, так и через TLS;
- на этапе handshake клиент передаёт технические app credentials, сначала пробует текущий v2-handshake и только при явном разрешении может откатиться на legacy-ветку;
- далее весь прикладной обмен идёт бинарными пакетами Aegis: авторизация, история сообщений, реакции, пины, состав участников, настройки комнат и события в реальном времени;
- после успешной авторизации сервер возвращает реальный opaque session token, который клиент сохраняет и использует для восстановления TCP-сессии без повторного ввода пароля;
- TOTP 2FA поддерживается end-to-end: код или recovery phrase можно использовать при входе, а настройка и отключение выполняются через API;
- сообщения, которые не удалось отправить из-за обрыва соединения или недоступности сервера, ставятся в локальную офлайн-очередь и автоматически переотправляются после восстановления сессии;
- часть возможностей уже зависит от того, какие handler'ы включены на сервере, поэтому некоторые сценарии в альфа-сборках могут быть помечены как недоступные или находящиеся в доработке.

В репозитории клиента мы специально не публикуем лишние внутренние детали протокола и ключевой материал. Для локальной разработки и CI достаточно корректно заполнить env-переменные, описанные в [.env.example](.env.example).
...
- `AEGIS_BOT_API_BASE_URL` - укажите явно, если Bot API с TOTP живёт не на стандартном `:5000` рядом с TCP-сервером.
...
Для 2FA/TOTP клиент ожидает рабочие HTTP endpoints Bot API:

- `/api/auth/2fa/setup`
- `/api/auth/2fa/enable`
- `/api/auth/2fa/disable`

Они вызываются с заголовком `X-Session-Token`, который берётся из активной Aegis-сессии.
...
Установка:
- Скачайте ZIP-архив из GitHub Actions → Artifacts
...
- Скачайте ZIP-архив из GitHub Actions → Artifacts и распакуйте
- Запуск:
