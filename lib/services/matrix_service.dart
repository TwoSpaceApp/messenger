...
/// dozens of call sites while switching runtime behavior to Matrix.
///
/// This implementation keeps signatures and return types compatible but
/// implements functionality via Matrix (ChatMatrixService) or lightweight
/// local fallbacks. Methods that cannot be sensibly implemented without a
/// server are implemented as simple failures or no-ops; extend as needed.
class MatrixService {
  MatrixService._();

  // Minimal cached token and user id to keep compatibility with callers.
  ...
  static Future<void> restoreJwt() async {
    try {
      final saved = await SecureStore.read('matrix_jwt');
  ...
  static Uri getFileViewUrl(String fileId, {String? bucketId}) {
    ...
    final resolved = (bucketId != null && bucketId.isNotEmpty) ? bucketId : Environment.matrixStorageMediaBucketId;
  ...
  static bool get isConfigured => Environment.useMatrix || v1Endpoint().isNotEmpty;
