...
class ExportService {
  ...
  Future<String> exportChatAsJson(String chatId, String chatName,
    ...
    final fileName =
        'chat_${chatId}_${DateTime.now().millisecondsSinceEpoch}.json';
  ...
  Future<String> exportMultipleChatsAsJson(
      List<Map<String, dynamic>> chatsData) async {
    final dir = await getApplicationDocumentsDirectory();
    final exportData = {
      'exportedAt': DateTime.now().toIso8601String(),
    ...
    final fileName =
        'twospace_backup_${DateTime.now().millisecondsSinceEpoch}.json';
    final file = File('${dir.path}/$fileName');
    await file.writeAsString(jsonEncode(exportData));
  ...
  Future<String> exportChatAsCsv(String chatId, String chatName,
      List<Map<String, dynamic>> messages) async {
    final dir = await getApplicationDocumentsDirectory();
    ...
    final fileName =
        'chat_${chatId}_${DateTime.now().millisecondsSinceEpoch}.csv';
    final file = File('${dir.path}/$fileName');
