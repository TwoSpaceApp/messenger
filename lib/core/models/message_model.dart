...
class MessageModel {
  ...
  factory MessageModel.fromMap(Map<String, dynamic> m) {
    return MessageModel(
      id: m[r'$id']?.toString() ?? m['id']?.toString() ?? '',
      ...
      reactions: (m['reactions'] is List)
          ? List<String>.from(m['reactions'])
          : <String>[],
      deliveredTo: (m['deliveredTo'] is List)
          ? List<String>.from(m['deliveredTo'])
          : <String>[],
      readBy:
          (m['readBy'] is List) ? List<String>.from(m['readBy']) : <String>[],
      replyToMessageId: m['replyToMessageId']?.toString(),
      replyToPreview: (m['replyToPreview'] is Map)
          ? Map<String, dynamic>.from(m['replyToPreview'])
          : null,
