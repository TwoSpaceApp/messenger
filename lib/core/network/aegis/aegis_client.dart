...
class AegisClient {
	...
	Future<MemberPermissionUpdateResponse> updateMemberPermissions({
		...
		return MemberPermissionUpdateResponse.fromJson(_decodeMap(response.payload));
	}
	...
	Future<void> sendDeliveryReceipt(
		...
		_ensureAuthenticated();
		final ids = messageIds.where((id) => id > 0).toSet().toList(growable: false);
		if (ids.isEmpty) {
			return;
		}
		await _sendRequest(
	...
	Future<void> _sendHandshake() async {
		final payload = <int>[];
		payload.addAll(_int32ToBytes(
			ProtocolConstants.versionMajor * 1000 + ProtocolConstants.versionMinor,
		));
		payload.addAll(_generateNonce());
	...
	Future<AuthResponsePayload> _authenticateRaw({
		...
		final authResponse = AuthResponsePayload.fromJson(_decodeMap(response.payload));
